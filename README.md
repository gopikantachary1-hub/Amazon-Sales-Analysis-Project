...
1. Top Selling Products Query: Query the top 10 products by total sales value.
...
Challenge: Display the sales trend, grouping by month; return current_month_sale and last_month_sale.
...
5. Customers with No Purchases: Find registered customers who never placed an order.
...
6. Least-Selling Categories by State: Identify the least-selling product category for each state.
...
7. Customer Lifetime Value (CLTV): Calculate total value and rank customers based on lifetime spend.
...
8. Inventory Stock Alerts: Query products with stock levels below a certain threshold (e.g., less than 10 units). Include last restock date and warehouse information.
...
9. Shipping Delays: Identify orders where shipping took more than 3 days.
...
10. Payment Success Rate: Calculate the percentage of successful payments and breakdown by status.
...
11. Top Performing Sellers: Find the top 5 sellers based on total sales value and display their percentage of successful orders.
...
12. Product Profit Margin: Rank products by their profit margin percentage.
...
SELECT 
    ...
    SUM(CASE WHEN o.order_status = 'Returned' THEN 1 ELSE 0 END)::numeric/COUNT(*)::nume...
FROM order_items as oi
JOIN 
products as p ON oi.product_id = p.product_id
...
Challenge: Return product_id, product_name, category_name, 2022 revenue and 2023 revenue decrease ratiobat end Round the result Note: Decrease ratio = cr-ls/ls* 100 (cs = current_year ls=last_year)
...
19. Final Task: Stored Procedure Create a stored procedure that, when a product is sold, performs the following actions: Inserts a new sales record into the orders and order_items tables. Updates the inventory table to reduce
the stock based on the product and quantity purchased. The procedure should ensure that the stock is adjusted
immediately after recording the sale.
...
-- checking stock and product availability in inventory
    ...
    -- update inventory
        ...
        INSERT INTO order_items(order_item_id, order_id, product_id, quantity, price, total_sale)
        VALUES
        ...
        RAISE NOTICE 'Thank you product: %% sale has been added also inventory stock updated', v_product;
...
Conclusion:
This project demonstrates the ability to solve e-commerce problems—from improving customer retention to optimizing logistics—using structured queries to drive business decision-making.



Here is a 3-point bulleted summary of your Amazon e-commerce analysis project:


Advanced Data Analysis: Analyzed over 20,000 sales records using PostgreSQL to solve complex business problems, including revenue forecasting, customer segmentation, and identifying sales trends.


Database Design & Optimization: Designed and implemented a normalized database schema with an ERD, utilizing advanced SQL techniques like window functions, CTEs, and stored procedures for efficient inventory management.
+2


Operational Problem Solving: Conducted in-depth analysis to optimize logistics and customer retention by cleaning real-world datasets and handling critical null values to improve business decision-making.
